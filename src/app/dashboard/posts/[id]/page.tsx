...
export default function EditPostPage() {
  ...
  useEffect(() => {
    ...
    authApi(session.accessToken).get(`/posts/${id}`)
      .then(({ data }) => {
        setTitle(data.title)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("")
    try {
      await authApi(session?.accessToken as string).put(`/posts/${id}`, { title, content })
  ...
  if (fetching) {
    return (
      <div className="max-w-2xl">
        <div className="h-8 w-24 bg-slate-100 rounded mb-8 animate-pulse" />
